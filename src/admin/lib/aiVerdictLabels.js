...
const FAILURE_INFO = {
  is_receipt: {
    ...
    hint:     'We couldn’t find a printed store receipt in your photo.',
  ...
  is_authentic_burger_king: {
    ...
    long:  'Wrong brand',
    icon:  '🏷️',
    title:    "Not a valid {brand} receipt",
    hint:     'We couldn’t confirm the brand, menu items, and printed total.',
  ...
  contains_required_item: {
    ...
    long:  'Wrong item',
    icon:  '🍔',
    title:    "Reward item not on the receipt",
    hint:     'The receipt has to include the item you’re claiming.',
    nextStep: 'Buy the reward item and upload the new receipt from that purchase.',
  },
  is_newer_than_cup_return: {
    short: 'Dated before unlock',
    long:  'Dated before the reward was unlocked',
    icon:  '🔓',
    title:    'This receipt is dated before the reward was unlocked',
    hint:     'It’s dated before you last collected enough cups to unlock this reward.',
    nextStep: 'Buy the reward item after you’ve unlocked it, and upload that new receipt.',
  },
  price_mismatch: {
    short: 'Price differs',
    long:  'Receipt price differs from ours',
    icon:  '💶',
    title:    'Receipt price doesn’t match',
    hint:     'The price on the receipt is different from the reward’s price in our system.',
    nextStep: 'Check the receipt shows the right item at the expected price.',
  },
  within_claim_window: {
    short: 'Outside window',
    long:  'Claimed too long after purchase',
    icon:  '⏳',
    title:    'This receipt is outside the claim window',
    hint:     'Claims must be made within the allowed number of days after purchase.',
    nextStep: 'Buy the reward item and claim it within the allowed window.',
  },
  duplicate_receipt: {
    short: 'Duplicate',
    long:  'Already used',
...
const PASS_LABELS = {
  ...
  price_mismatch:           'Price matches ours',
  is_newer_than_cup_return: 'Dated after reward unlocked',
  within_claim_window:      'Within claim window',
...
export function getFailureCopy(code, opts = {}) {
  ...
  if (!info) {
    return {
      icon:  '⚠️',
      title: "Couldn't verify your receipt",
