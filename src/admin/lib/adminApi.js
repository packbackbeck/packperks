...
export async function getTikkieStatsMetrics({ fromTs = null, toTs = null, orgIds } = {}) {
  ...
  const inRange = (q, col) => {
    if (fromTs) q = q.gte(col, new Date(fromTs).toISOString());
    if (toTs)   q = q.lte(col, new Date(toTs).toISOString());
    return q;
  };
  ...
  const [claimsRes, pendingRes, backupRes, sessionsRes] = await Promise.all([
    ...
    inRange(
      supabase.from('pending_batches')
        .select('batch_id, org_id, email, user_id, first_seen, resolved_at')
        .or(ids.length ? `org_id.is.null,org_id.in.(${ids.join(',')})` : 'org_id.is.null'),
      'first_seen'
    ),
    inRange(applyOrgFilter(
      supabase.from('backup_cup_uses').select('id, claim_id, used_at'), orgIds
    ), 'used_at'),
    inRange(applyOrgFilter(
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: pct(pendingDone, pendingAll), numerator: pendingDone, denominator: pendingAll,
      lowerIsBetter: false, thresholds: { go: 90, condLow: 70 },
      formula: 'Pending receipts the bin later confirmed / receipts scanned before confirmation',
      note: pendingAll === 0
        ? 'No receipt has been scanned before its bin session arrived — the bin is delivering sessions ahead of the customer.'
        : 'Low means the bin is losing sessions: printed receipts whose confirmation never arrives.',
    },
    {
      id: 'tk_backup', label: 'Backup fallback rate',
      value: total > 0 ? pct(backupSet.size, total) : null, numerator: backupSet.size, denominator: total,
      lowerIsBetter: true, thresholds: { go: 0, condHigh: 15 },
      formula: 'Payouts from reserved backup cups / receipts scanned',
      note: backupSet.size > 0 ? 'Anything above zero means the bin was offline — see Backup Cups.' : null,
    },
    {
      id: 'tk_uptime', label: 'Payout uptime',
      value: activeHours > 0 ? (cleanHours / activeHours) * 100 : null,
      numerator: cleanHours, denominator: activeHours,
      lowerIsBetter: false, thresholds: { go: 95, condLow: 85 },
      formula: 'Active hours with zero mint failures / active hours',
      note: 'Proxy: hours with at least one receipt and no failed mint.',
    },
  ].map(m => ({
    ...m,
    band: classify({
  ...
  for (const c of claims) {
    ...
    e.count += 1;
    if (!e.lastSeen || c.created_at > e.lastSeen) {
      e.lastSeen = c.created_at;
      e.lastMessage = c.tikkie_last_error;
    }
    errMap.set(code, e);
  }
  const errorBreakdown = Array.from(errMap.values()).sort((a, b) => b.count - a.count);
  ...
  return {
    ...
    totals: {
      ...
      lastScan: claims[claims.length - 1]?.created_at || null,
    },
  };
}
 ...
 * Wallet model: returns CREDIT a profile's balance (claims rows keyed by
 * batch_id) and Tikkie links are minted per bulk redemption (claims rows
 * with no batch). The audience splits into ACCOUNTS (profiles that saved
 * an email) and plain PROFILES (device-only).
 * ───────────────────────────────────────────────────────────────────── */
const TIKKIE_TS = {
  claims:  r => r.created_at,
  users:   r => r.created_at,
...
function computeTikkieMetrics({ claims, users, pending, backup, rej = [] }) {
  ...
  // Credits (one per scanned receipt) vs link rows (legacy per-receipt
  // links + wallet-era bulk payouts). Money totals come from CREDITS only:
  // a payout row re-states the sum of the credits it swept.
  const credits      = claims.filter(c => c.batch_id);
  const linkRows     = claims.filter(c => c.tikkie_url);
  const receipts     = credits.length;
  const links        = linkRows.length;
  const redeemed     = linkRows.filter(c => c.tikkie_status === 'redeemed').length;
  const expired      = linkRows.filter(c => c.tikkie_status === 'expired').length;
  const attached     = credits.filter(c => c.user_id).length;
  const profilesAll  = users.length;
  const accounts     = users.filter(u => u.email).length;
  const profilesOnly = profilesAll - accounts;
  const emailOnly    = pending.filter(p => p.email && !p.user_id).length;
  const reachable    = accounts + emailOnly;
  const pendingAll   = pending.length;
  const pendingDone  = pending.filter(p => p.resolved_at).length;
  const backupClaims = new Set(backup.map(b => b.claim_id).filter(Boolean)).size;
  const totalPaid    = credits.reduce((s, c) => s + Number(c.payout_amount || 0), 0);
  const totalCups    = credits.reduce((s, c) => s + Number(c.cups_redeemed || 0), 0);
  ...
  return [
    ...
    M({ id: 'tk_email_capture', group: 'primary', label: 'Email capture rate',
        ...
        numLabel: 'Emails captured', denLabel: 'Receipts scanned',
        desc: 'How often a scanned receipt turns into someone we can reach by email: an account, or an email left on the held-for-review popup.' }),
    { measurable: true, id: 'tk_audience', group: 'primary', label: 'Profiles vs accounts',
        valueType: 'count', value: null, rawValue: profilesAll, valueText: profilesAll.toLocaleString(),
        numerator: profilesAll, numLabel: 'Refund profiles', denominator: null, denLabel: null,
        breakdown: [
          { key: 'accounts', label: 'Accounts', count: accounts },
          { key: 'profiles', label: 'Profiles', count: profilesOnly },
        ],
        breakdownTitle: 'Profiles vs accounts', breakdownNoun: 'people',
        desc: 'Every scanner gets a profile on their first scan. Accounts are profiles that saved an email, so their balance survives a lost phone; plain profiles live on one device only.' },
    M({ id: 'tk_account_conv', group: 'primary', label: 'Account conversion',
        numerator: accounts, denominator: profilesAll,
        numLabel: 'Accounts (email saved)', denLabel: 'All profiles',
    ...
    M({ id: 'tk_attached', group: 'secondary', label: 'Refunds on profiles',
        ...
        desc: 'Receipts credited to a profile\'s balance (visible in that customer\'s history). Wallet-era scans always attach; a gap means legacy anonymous refunds.' }),
...
/* Tikkie-only payout log: every Tikkie link we minted. Wallet-era bulk
 * payouts have no batch_id (they sweep many receipts); legacy per-receipt
 * links keep theirs. Failed mint attempts surface via tikkie_last_error. */
