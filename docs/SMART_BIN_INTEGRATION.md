...
new batch as if it were fresh.

Only the cup registration is released. The claims the old batch created
keep their amount and cup count, so customer balances and history are
untouched — but the old printed receipt is spent: scanning it returns
`batch_expired`, and the customer is told the receipt is no longer valid
