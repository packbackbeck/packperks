...
const MAX_SCAN = 3000;   // elements a layout snapshot will look at
...
/* Every control on the screen right now, for the wireframe. */
...
/* Where the controls sat, as fractions of the page. The heatmap draws
 * this as a wireframe so a hotspot can be read without ever holding a
 * picture of a customer's screen. One snapshot per screen per visit. */
function snapshotLayout() {
  ...
  const nodes = pressableNodes().slice(0, 200);
  const elements = [];
