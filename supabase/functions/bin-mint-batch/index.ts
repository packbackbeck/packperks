...
// calls this endpoint ONCE at the end of the session. We mint a batch of
// cup tokens, and return the URL the bin prints as a QR on the receipt:
//
...
Deno.serve(async (req) => {
  ...
  // Body: { cups, session_id }. `count` is accepted as a legacy alias so an
  // already-flashed bin keeps working without a firmware change.
  let cups = 1;
  let sessionId = "";
  try {
    const b = await req.json();
    const raw = b?.cups ?? b?.count;
    if (Number.isFinite(Number(raw))) cups = parseInt(String(raw), 10);
    if (b?.session_id != null) sessionId = String(b.session_id).trim().slice(0, 128);
  } catch { /* defaults */ }

  if (!Number.isFinite(cups) || cups < 1 || cups > MAX_CUPS_PER_SESSION) {
    return json({ error: "invalid_cups", detail: `cups must be 1..${MAX_CUPS_PER_SESSION}` }, 400);
  }
  ...
  admin.from("system_events").insert({
    ...
    status: "ok",
    count: minted,
    detail: { machine_id: machineId, batch_id: batchId, amount_eur: amount, session_id: sessionId || null },
